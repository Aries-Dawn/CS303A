...
class AI(object):
    ...
    def go(self, chessboard):
        ...
        result = []
        for idx_temp in idx:
            judgement, eat = self.judge(idx_temp[0], idx_temp[1], chessboard)
            if judgement:
                result.append((eat, idx_temp[0], idx_temp[1]))
        judgement, lists = self.has_corner(result)
        if judgement:
            for idx_temp in result:
                self.candidate_list.append((idx_temp[1], idx_temp[2]))
            self.candidate_list[len(self.candidate_list) - 1] = (lists[0][1], lists[0][2])
        else:
            ...
            for i in result:
                ...
                if m < i[0]:
                    mx, my, m = x, y, i[0]
                if not self.dead(x, y):
                    self.candidate_list.append((x, y))
            if len(self.candidate_list) == 0 and len(result) != 0:
                self.candidate_list.append((mx, my))
        # print(result)
